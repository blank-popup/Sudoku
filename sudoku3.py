...
def solve_all(grids, name='', showif=0.0):
    ...
    N = len(grids)
    if N > 1:
        print(f'Solved {sum(results)} of {N} {name} puzzles (avg {sum(times)/N:.2f} secs ({N/sum(times)} Hz), max {max(times):.2f} secs).')
...
def show_problem_solution(problem):
    display(grid_values(problem))
    solution = solve(problem)
    display(solution)

if __name__ == '__main__':
    # solve_all([grid1])
    show_problem_solution(hard1)
    ...
    # solve_all(grid2, 'hard', None)
    # solve_all(hard1, 'hardest', None)
...
# # https://github.com/neolee/sudoku-norvig
